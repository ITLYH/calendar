  ...
  <div id="comments">
    <div id="comments_box">
      <div class="tab_barList_video_box">
        <div
          class="img_tiezi"
          v-show="this.items.url.split('.')[this.items.url.split('.').length-1] == 'jpg'"
        >
        ...
        <img
          ...
          height="215"
          width="auto"
          v-if="this.items.url.split('.')[this.items.url.split('.').length-1] == 'jpg'"
        />
      ...
      <div class="tab_barList_content">
        ...
        <div class="content_right" @click="GuanZuFun">
          <span
            class="guanZu"
            :style="{'color':isGuanZu?'red':'#fff'}"
            v-text="isGuanZu?'已关注':'关注'"
          ></span>
        </div>
      </div>
    </div>
        ...
        <div>
          <div>
            ...
            <span @click="giveLike(item.comments_good,index,$event)">
              ...
              <i
                :class="item.flag?'iconfont icon-good-filling':'iconfont icon-good'"
                :style="{'color':item.flag?'red':'black'}"
              ></i>
            </span>
          </div>
          <p>{{item.comments_detail}}</p>
          <span @click="popupVisible4=true">
  ...
  methods: {
    ...
    giveLike(num, index, e) {
      ...
      iconGood.style.color == "red"
        ? (this.commentsList[index].flag = false)
        : (this.commentsList[index].flag = true);
    ...
    checkFo() {
      if (document.addEventListener) {
        document
          .getElementById("user_reply_text")
          .addEventListener("blur", function() {
            document.getElementsByClassName("user_reply")[0].style.bottom =
              "0px";
          });
      } else {
        document
