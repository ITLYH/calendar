<template>
    <div id="comments">
        ...
        <div class="statistical_log">
            <p>
                <span class="iconfont icon-discount"></span> 吃鸡小分队
            </p>
            <div>
                <div>
                    <span>{{this.items.look}}次观看</span>
                </div>
                <div>
                    <span>{{this.items.attention}}人关注</span>
        ...
        <div class="comments_list" ref="comments_top">
            <div class="comments_listS" v-for="(item,index) in commentsList" :key="index">
                ...
                <div>
                    ...
                    <span @click="showCommentsDetail(item)">
                        {{item.comments_reply}}回复
        ...
        <!-- 回复详情弹窗 -->
        <van-action-sheet v-model="show" title="回复详情" :round="true">
            <div class="tab_barList_content">
                <div class="content_left">
                    <img class="user_img" :src="moreComments.comments_user_img" alt />
                    <div class="user_msg">
                        <span>{{moreComments.comments_user}}</span>
                        <!-- <br />
                        <span>{{moreComments.describe}}</span> -->
                    </div>
                </div>
                <div class="content_right" @click="GuanZuFun(moreComments.isFocus)">
                    <span class="guanZu" :style="{'color':moreComments.isFocus?'red':'#fff'}" v-text="moreComments.isFocus?'已关注':'关注'"></span>
                </div>
            </div>

            <div class="comments_content">
                <p class="log_title_action">{{moreComments.comments_detail}}</p>
                <div>
                    <span>500人插眼</span>
                    <i class="iconfont icon-attachment"></i>
                </div>
...
export default {
    ...
    methods: {
        ...
        toTop() {
            if (
                this.items.url.split(".")[this.items.url.split(".").length - 1] == "mp4"
            ) {
